...
% Start by creating the matrix with bias as the first column to create a1
% Note that X is our 'a1' for inputs.
X = [ones(m, 1) X]; % 5000 x 401 matrix

% Now, we need to create z2, which is Theta1 * a1'.
z2 = Theta1 * X'; % (24x401) x (401x5000)

% Take z2 and put it into the sigmoid function for logistic regression
a2 = sigmoid(z2);
...
h_theta = sigmoid(z3); % This is a3

% recode the labels as vectors containing only values 0 or 1 (page 5 of ex4.pdf)
% in this for loop, we are checking the output vector y, and placing a 1 a
% true in y_new, which is corresponding to what the answer is.
